...
class PCAMAD():
    ...
    def __init__(self, n_components=2, _evalvariant=0, seed=0, verbose=0, trn_tst_split=False):
        super(PCAMAD, self)
    ...
    def fit(self, x):
        ...
        if self.n_components is None:
            print(f"PCAMAD: n_components is None -> n_components={x.shape[1]}")
            self.n_components = x.shape[1]
            
        if self.n_components < 2:
            print(f"PCAMAD: n_components={self.n_components} below minimum -> updated to n_components=2")
            self.n_components = 2
    ...
    def predict(self, x):
        ...
        self.decision_scores_ = np.nan_to_num(y_preds)
    ...
    def calculate_outlier_scores(self, S_pr):
        ...
        outlier_scores = (self.eigenvalues * S_pr).sum(axis=1) # to check: sum positive/negative values or abs values only?
    ...
    def calculate_corrected_outlier_scores(self, S_pr):
        ...
        outlier_scores = (eigenvalues_corrected * S_pr).sum(axis=1) # to check: sum positive/negative values or abs values only?
